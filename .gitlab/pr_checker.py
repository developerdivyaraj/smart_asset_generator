#!/usr/bin/env python3
"""
GitLab Merge Request Checker - GetX Project
...
class GetxMrChecker:
    """Handles all MR validation checks for GetX Project Flutter project."""
    ...
    def check_broken_code_patterns(self, changes: List[Dict]) -> Tuple[bool, str, List[str]]:
        ...
        # GetX Project project's specific flutter_screenutil rules
        ...
        broken_patterns = [
            ...
            # GetX Project specific EdgeInsets rules (excluding 0)
        ...
        if not violations:
            ...
            if len(checked_file_names) > 5:
                files_list += f" and {len(checked_file_names) - 5} more"
            return True, f"✅ **Flutter ScreenUtil**: All patterns follow GetX Project conventions ({checked_files} files checked: {files_list})", issues
        else:
            violations_list = '\n'.join(violations[:10])
            if len(violations) > 10:
                violations_list += f"\n  - ... and {len(violations) - 10} more"
            issues.append("GetX Project flutter_screenutil rules:")
    ...
    def run_all_checks(self) -> bool:
        ...
        print("\n" + "="*70)
        print("🔍 GetX Project - GitLab MR Checker")
        print("="*70 + "\n")
        ...
        comment = f"""## 🤖 GetX Project Code Quality Check
...
*Generated by GetX Project GitLab CI MR Checker*
"""
